...
"[project]/node_modules/@3d-dice/dice-box/dist/dice-box.es.js [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var F = (c, l, b)=>(Yl(c, typeof l != "symbol" ? l + "" : l, b), b), g = (c, l, b)=>{
    if (!l.has(c)) throw TypeError("Cannot " + b);
};
var W = (c, l, b)=>(g(c, l, "read from private field"), b ? b.call(c) : l.get(c)), Y = (c, l, b)=>{
    if (l.has(c)) throw TypeError("Cannot add the same private member more than once");
...
function il() {
    let c;
    try {
        if (c = Xl && (window.URL || window.webkitURL).createObjectURL(Xl), !c) throw "";
        return new Worker(c);
    } catch (e) {
...
const pl = (c)=>{
    ...
    const { dedupe: l } = c;
    let b = [], d;
    const X1 = (m)=>(l && (b = []), b.push(m), d || (d = Z()), d.finally(()=>{
            d = void 0;
        })), Z = async ()=>{
        const m = [];
        for(; b.length;){
...
const al = (c)=>{
    ...
    b.length === 3 ? b = [
        ...b
    ].map((X1)=>X1 + X1).join("") : b.length === 8 && (l = !0), b = parseInt(b, 16);
    let d = {
        r: b >>> 16,
        g: (b & 65280) >>> 8,
        b: b & 255
...
const sl = {
    id: "dice-canvas-".concat(Date.now()),
    // set the canvas id
    container: null,
    enableShadows: !0,
...
class vl {
    ...
    async init() {
        return W(this, y) ? v(this, E, Gl).call(this) : i(this, K, Promise.resolve()), await v(this, I, cl).call(this), this.resizeWorld(), W(this, u).onRollResult = (l)=>{
            const b = this.rollDiceData[l.rollId], d = this.rollGroupData[b.groupId], X1 = this.rollCollectionData[b.collectionId];
            d.rolls[b.rollId].value = l.value, X1.completedRolls++, X1.completedRolls == X1.rolls.length && X1.resolve(Object.values(X1.rolls).map((param)=>{
                let { collectionId: G, id: J, meshName: R, ...N } = param;
                return N;
            }));
            ...
            const X1 = this.rollGroupData[b.groupId];
            delete X1.rolls[b.rollId];
            const Z = v(this, P, r).call(this, b.groupId);
            X1.value = Z.value, X1.qty = Z.rollsArray.length, d.completedRolls == d.rolls.length && d.resolve(Object.values(d.rolls).map((param)=>{
                let { id: N, ...L } = param;
                return L;
            }));
            ...
            W(this, w),
            W(this, K)
        ]), W(this, p) && v(this, T, Wl).call(this), await this.loadThemeQueue.push(()=>this.loadTheme(this.config.theme)), this.config.preloadThemes.forEach((async function(l) {
    ...
    async getThemeConfig(l) {
        let b = "".concat(this.config.origin).concat(this.config.assetPath, "themes/").concat(l);
        this.config.externalThemes[l] && (b = this.config.externalThemes[l]);
        let d = await fetch("".concat(b, "/theme.config.json")).then((m)=>{
            if (m.ok) {
                const V = m.headers.get("content-type");
                if (V && V.indexOf("application/json") !== -1) return m.json();
                if (m.type && m.type === "basic") return m.json();
                throw new Error("Incorrect contentType: ".concat(V, '. Expected "application/json" or "basic"'));
            } else throw new Error("Unable to fetch config file for theme: '".concat(l, "'. Request rejected with status ").concat(m.status, ": ").concat(m.statusText));
        }).catch((m)=>console.error(m));
        if (!d) throw new Error("No theme config data to work with.");
        let X1 = "default", Z = "".concat(this.config.origin).concat(this.config.assetPath, "themes/default/default.json");
        if (d.hasOwnProperty("meshFile") && (X1 = d.meshFile.replace(/(.*)\..{2,4}$/, "$1"), Z = "".concat(b, "/").concat(d.meshFile)), !d.hasOwnProperty("diceAvailable")) throw new Error('A theme must indicate which dice are available by defining "diceAvailable".');
        if (d.hasOwnProperty("extends")) {
            const m = await this.loadTheme(d.extends).catch((G)=>console.error(G));
            if (m.hasOwnProperty("extends")) throw new Error("Cannot extend a theme that extends another theme.");
            const V = {};
    ...
    async loadTheme(l) {
        if (this.themesLoadedData[l]) return this.themesLoadedData[l];
        const b = this.themesLoadedData[l] = await this.getThemeConfig(l).catch((d)=>console.error(d));
        if (this.onThemeConfigLoaded(b), !!b) return await W(this, u).loadTheme(b).catch((d)=>console.error(d)), this.onThemeLoaded(b), b;
    }
    ...
    async updateConfig(l) {
        ...
        if (this.config = b, b.theme) {
            const X1 = (await this.loadThemeQueue.push(()=>this.loadTheme(b.theme))).at(-1);
            X1.hasOwnProperty("extends") && (this.config.theme = X1.extends);
        }
    ...
    clear() {
        return i(this, f, 0), i(this, x, 0), i(this, C, 0), i(this, B, 0), this.rollCollectionData = {}, this.rollGroupData = {}, this.rollDiceData = {}, W(this, u).clear(), W(this, p) && W(this, p).postMessage({
            action: "clearDice"
        }), this;
    }
    hide(l) {
    ...
    roll(l) {
        let { theme: b = this.config.theme, themeColor: d = this.config.themeColor, newStartPoint: X1 = !0 } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
        this.clear();
        const Z = o(this, f)._++;
    ...
    reroll(l) {
        let { remove: b = !1, hide: d = !1, newStartPoint: X1 = !0 } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
    ...
    remove(l) {
        let { hide: b = !1 } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
        const d = Array.isArray(l) ? l : [
            l
        ], X1 = o(this, f)._++;
        return this.rollCollectionData[X1] = new M({
            id: X1,
            notation: l,
            rolls: d
        }), d.map((Z)=>{
            this.rollDiceData[Z.rollId].removeCollectionId = X1;
        ...
        }), this.rollCollectionData[X1].promise;
    }
    ...
    createNotationArray(l, b) {
        ...
        let X1 = [];
        const Z = (G)=>{
            if (G.hasOwnProperty("qty") || (G.qty = 1), G.hasOwnProperty("sides")) return G.sides === "100" && (G.sides = 100, G.data = "single"), !0;
            {
        ...
        return d.forEach((G)=>{
            typeof G == "string" ? X1.push(this.parse(G, b)) : typeof d == "object" && (V(G), Z(G) && X1.push(G));
        }), X1;
    }
    // parse text die notation such as 2d10+3 => {number:2, type:6, modifier:3}
    // taken from https://github.com/ChapelR/dice-notation
    parse(l, b) {
        const d = /(\d+)([dD]{1}\d+)(.*)$/i, X1 = /(\d+)[dD](00|%)(.*)$/i, Z = /(\d+)[dD](f+[ate]*)(.*)$/i, m = /(\d+)[dD]([\d\w]+)([+-]{0,1}\d+)?/i, V = /([+-])(\d+)/, G = l.trim().replace(/\s+/g, ""), J = (h, S)=>{
            if (h = Number(h), Number.isNaN(h) || !Number.isInteger(h) || h < 1) throw new Error(S);
            return h;
        }, R = G.match(X1) || G.match(d) || G.match(Z) || G.match(m);
        let N = 0;
        const L = "Invalid notation: " + l;
        if (!R || !R.length || R.length < 3) throw new Error(L);
        if (R[3] && V.test(R[3])) {
            const h = R[3].match(V);
            let S = J(h[2], L);
            h[1].trim() === "-" && (S *= -1), N = S;
        ...
        return G.match(X1) ? (s.sides = "d100", s.data = "single") : G.match(Z) ? s.sides = "fate" : (b.includes(G.match(m)[2]), s.sides = R[2]), s;
    }
    getRollResults() {
        return Object.entries(this.rollGroupData).map((param)=>{
            let [l, b] = param;
            ...
            return X1.rolls = d.rollsArray, X1;
        });
    }
    constructor(l = {}){
...
f = new WeakMap(), x = new WeakMap(), C = new WeakMap(), B = new WeakMap(), u = new WeakMap(), w = new WeakMap(), U = new WeakMap(), p = new WeakMap(), K = new WeakMap(), k = new WeakMap(), y = new WeakMap(), I = new WeakSet(), cl = async function() {
    ...
    if (W(this, y)) if ("OffscreenCanvas" in window && "transferControlToOffscreen" in this.canvas && this.config.offscreen) {
        const b = await __turbopack_context__.A("[project]/node_modules/@3d-dice/dice-box/dist/world.offscreen.js [app-client] (ecmascript, async loader)").then((d)=>d.default);
        i(this, u, new b({
            canvas: this.canvas,
            options: this.config,
            onInitComplete: l
        ...
        const b = await __turbopack_context__.A("[project]/node_modules/@3d-dice/dice-box/dist/world.onscreen.js [app-client] (ecmascript, async loader)").then((d)=>d.default);
        i(this, u, new b({
            canvas: this.canvas,
            options: this.config,
            onInitComplete: l
        ...
        const b = await __turbopack_context__.A("[project]/node_modules/@3d-dice/dice-box/dist/world.none.js [app-client] (ecmascript, async loader)").then((d)=>d.default);
        i(this, u, new b({
            canvas: this.canvas,
            options: this.config,
            onInitComplete: l
    ...
    let X1 = d.newStartPoint;
    l.forEach(async (Z)=>{
        var q, $, _, ll, bl, dl;
        if (!Z.sides) throw new Error("Improper dice notation or unable to parse notation");
        ...
        for(var A = 0, Vl = Z.qty; A < Vl; A++){
            ...
            const H = Number.isInteger(Z.sides) ? "d".concat(Z.sides) : Z.sides;
            ...
            if (G[e] = a, this.rollDiceData[e] = a, d.rolls.push(this.rollDiceData[e]), a.sides === "fate" && !s.includes(H) && !t.includes(H) || a.sides === "fate" && !W(this, y)) {
                console.warn("fate die unavailable in '".concat(m, "' theme. Using fallback."));
                const n = -1, z = 1;
                a.value = Zl.range(n, z), W(this, u).addNonDie(a);
            } else if (this.config.suspendSimulation || !s.includes(H) && !t.includes(H) || !W(this, y)) {
                const n = W(this, y) ? this.config.suspendSimulation ? "3D simulation suspended. Using fallback." : "".concat(a.sides, " die unavailable in '").concat(m, "' theme. Using fallback.") : "This browser does not support webGL. Using random number fallback.";
                console.warn(n);
    ...
    const b = this.rollGroupData[l], d = Object.values(b.rolls).map((param)=>{
        let { collectionId: Z, id: m, meshName: V, ...G } = param;
        return G;
    });
    let X1 = d.reduce((Z, m)=>{
    ...
    return X1 += b.modifier ? b.modifier : 0, {
        value: X1,
        rollsArray: d
    };
};
class M {
    constructor(l){
        ...
        const b = this;
        this.promise = new Promise((d, X1)=>{
            b.resolve = d, b.reject = X1;
        });
    }
}
;
...
//# sourceMappingURL=node_modules_%403d-dice_dice-box_dist_dice-box_es_a0389329.js.map