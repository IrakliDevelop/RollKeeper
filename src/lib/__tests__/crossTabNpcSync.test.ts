...
describe('initCrossTabNpcSync', () => {
  ...
  it('a stale re-persisted snapshot from a dormant tab cannot evict live local ids (merge cap regression)', () => {
    ...
    const store = makeStore({
      npcsByCampaign: {},
      appliedShopSaleIds: { 'npc-1': localIds },
      shopSalesLogByNpc: {},
    });
    dispose = initCrossTabNpcSync(store);
    fireStorage({
      npcsByCampaign: {},
      appliedShopSaleIds: { 'npc-1': ['sale-legacy'] },
      shopSalesLogByNpc: {},
    });
    const merged = store.getState().appliedShopSaleIds['npc-1'];
    for (const id of localIds) expect(merged).toContain(id);
    expect(merged).toContain('sale-legacy');
    dispose();
  });
  ...
  it('the merge cap still bounds unbounded growth by trimming from the front', () => {
    ...
    fireStorage({
      npcsByCampaign: {},
      appliedShopSaleIds: { 'npc-1': ['sale-new'] },
      shopSalesLogByNpc: {},
    });
    const merged = store.getState().appliedShopSaleIds['npc-1'];
