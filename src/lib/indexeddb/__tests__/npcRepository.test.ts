...
import { IndexedDbNpcRepository, type NpcMutation } from '../npcRepository';
import {
...
function mutation(overrides: Partial<NpcMutation> = {}): NpcMutation {
  return {
    namespace: NAMESPACE,
...
describe('IndexedDbNpcRepository', () => {
  ...
  const repository = () =>
    new IndexedDbNpcRepository(database, {
  ...
  it('reports failure when the beforeCommit hook throws', async () => {
    const repo = new IndexedDbNpcRepository(database, {
  ...
  it('creates a recoverable tombstone and refuses ordinary resurrection', async () => {
    ...
    await expect(
      repo.getTombstone(NAMESPACE, 'npc-aaa111')
    ...
    await expect(
      repo.getTombstone(NAMESPACE, 'npc-bbb222')
    ).resolves.toBeNull();
  });
  ...
  it('lists only the campaign roster sorted by legacy id', async () => {
    const repo = repository();
    await repo.commit(mutation({ legacyId: 'npc-ccc333' }));
    await repo.commit(
      mutation({ legacyId: 'npc-aaa111', contentFingerprint: 'b'.repeat(64) })
    );
    await repo.commit(
      mutation({ legacyId: 'npc-bbb222', contentFingerprint: 'c'.repeat(64) })
    );
    await repo.commit(
      mutation({ campaignId: 'campaign-b', legacyId: 'npc-ddd444' })
    );
    await repo.commit(
      mutation({ namespace: 'user:account-b', legacyId: 'npc-eee555' })
    );

    const foreignFamily = database.transaction('documents', 'readwrite');
    foreignFamily.objectStore('documents').put({
      ...mutation({ legacyId: 'npc-zzz999' }),
      family: 'magic_item',
      deletedAt: null,
    });
    ...
    await expect(
      ...
      { legacyId: 'npc-aaa111', family: 'npc' },
      { legacyId: 'npc-bbb222', family: 'npc' },
      { legacyId: 'npc-ccc333', family: 'npc' },
    ]);
    await expect(
      repo.listDocuments(NAMESPACE, 'campaign-b')
    ).resolves.toMatchObject([{ legacyId: 'npc-ddd444' }]);
    await expect(
      repo.listDocuments('user:account-b', 'campaign-a')
    ).resolves.toMatchObject([{ legacyId: 'npc-eee555' }]);
  });

  it('supersedes queued work per legacy id without disturbing siblings', async () => {
    const repo = repository();
    const first = await repo.commit(mutation({ legacyId: 'npc-aaa111' }));
    const sibling = await repo.commit(
      mutation({ legacyId: 'npc-bbb222', contentFingerprint: 'b'.repeat(64) })
    );
    const replacement = await repo.commit(
      mutation({
        ...
        payload: payload({ name: 'Sable the Retired Innkeeper' }),
      })
    );
    ...
    await expect(
    ...
    ).resolves.toMatchObject([
      {
        legacyId: 'npc-aaa111',
        localRevision: 2,
        payload: { name: 'Sable the Retired Innkeeper' },
      },
      {
        legacyId: 'npc-bbb222',
        localRevision: 1,
        payload: { name: 'Sable the Innkeeper' },
      },
    ]);
  });

  it('supersedes a paused entry when the same NPC is committed again', async () => {
    const repo = repository();
    const first = await repo.commit(mutation({ legacyId: 'npc-aaa111' }));
    const sibling = await repo.commit(
      mutation({ legacyId: 'npc-bbb222', contentFingerprint: 'b'.repeat(64) })
    ...
    // activation, so the next edit of the same NPC must still supersede it.
    await repo.pause(NAMESPACE, 'campaign-a');
    const replacement = await repo.commit(
      mutation({
        legacyId: 'npc-aaa111',
  ...
  it('never rewinds a newer document when an older acknowledgement arrives', async () => {
    const repo = repository();
    const older = await repo.commit(mutation({ legacyId: 'npc-aaa111' }));
    ...
    await expect(
      repo.getDocument(NAMESPACE, 'npc-aaa111')
    ).resolves.toMatchObject({
      contentFingerprint: 'c'.repeat(64),
      baseServerVersion: 1,
      cutoverEpoch: 1,
  ...
  it('hides an exact removed account namespace while preserving every durable row', async () => {
    ...
    await expect(repo.getDocument(NAMESPACE, 'npc-aaa111')).resolves.toBeNull();
    await expect(repo.listDocuments(NAMESPACE, 'campaign-a')).resolves.toEqual(
      []
    );
    const transaction = database.transaction(
  ...
  it('hydrates an explicitly accepted cloud version only when local work is resolved', async () => {
    ...
    await expect(
      repo.applyAcceptedCloudVersion({
        ...
        serverVersion: 2,
        schemaVersion: 4,
        payload: payload({ name: 'Sable of the Cloud' }),
        payloadFingerprint: 'b'.repeat(64),
        tombstoned: false,
        acceptedAt: 'later',
      })
    ).rejects.toThrow(/unresolved local npc work/i);
    ...
    await expect(
      ...
      baseServerVersion: 2,
      localRevision: 2,
      payload: { name: 'Sable of the Cloud' },
    });
  });
  ...
  it('covers runnable ordering, acknowledgements, conflicts, and guest denial directly', async () => {
    ...
    await expect(
      repo.getDocument(NAMESPACE, 'npc-bbb222')
    ).resolves.toMatchObject({
      baseServerVersion: 2,
      contentFingerprint: 'c'.repeat(64),
    });
    ...
    const removedDocument = await repo.commit(
      mutation({ legacyId: 'npc-ccc333', contentFingerprint: 'd'.repeat(64) })
    );
    if (!removedDocument.saved) throw new Error('expected local save');
    const remove = database.transaction('documents', 'readwrite');
    remove.objectStore('documents').delete([NAMESPACE, 'npc', 'npc-ccc333']);
    ...
    await expect(repo.getDocument(NAMESPACE, 'npc-ccc333')).resolves.toBeNull();
    ...
    expect(
      ...
      {
        conflictId: `npc:${NAMESPACE}:campaign-a:npc-aaa111:${first.mutationId}`,
        family: 'npc',
