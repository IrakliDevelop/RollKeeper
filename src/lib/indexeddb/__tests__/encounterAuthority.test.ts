...
} from '../encounterAuthority';
import type { EncounterDocument } from '../encounterRepository';
import {
...
function document(
  overrides: Partial<EncounterDocument> = {}
): EncounterDocument {
  return {
    namespace: NAMESPACE,
    campaignId: CAMPAIGN,
    legacyId: 'encounter-a',
...
async function seedReady(database: IDBDatabase) {
  ...
  transaction.objectStore('meta').put({
    key: `migration-state:${NAMESPACE}:encounter_definition:${CAMPAIGN}`,
    state: 'CUTOVER_READY',
    runId: GENERATION,
  });
...
describe('encounter local authority', () => {
  ...
  it('requires every safety gate and explicit confirmation', async () => {
    ...
    for (const field of Object.keys(gates) as Array<keyof typeof gates>) {
      await expect(
        commitEncounterLocalCutover(database, {
          namespace: NAMESPACE,
          campaignId: CAMPAIGN,
          generation: GENERATION,
          confirmed: true,
  ...
  it('atomically advances only the scoped family epoch', async () => {
    const database = await openRollkeeperDatabase();
    await seedReady(database);
    const authority = await commitEncounterLocalCutover(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      generation: GENERATION,
      confirmed: true,
    ...
    expect(await readEncounterAuthority(database, NAMESPACE, CAMPAIGN)).toEqual(
      authority
    );
    expect(await readEncounterAuthority(database, NAMESPACE, 'other')).toEqual({
    ...
    expect(
      await requestResult(
        meta
          .objectStore('meta')
          .get(`migration-state:${NAMESPACE}:encounter_definition:${CAMPAIGN}`)
      )
  ...
  it('writes every initial document inside the cutover transaction', async () => {
    ...
    await commitEncounterLocalCutover(database, {
      ...
      now: () => 'now',
      initialDocuments: [
        document({ legacyId: 'encounter-a' }),
        document({
          legacyId: 'encounter-b',
          payload: payload({ name: 'Cellar Skirmish' }),
    ...
    expect(stored.every(row => row.family === 'encounter_definition')).toBe(
      true
    );
    database.close();
  });
  ...
  it('rolls back through a new epoch only from a verified current generation', async () => {
    ...
    await expect(
      rollbackEncounterLocalAuthority(database, {
        namespace: NAMESPACE,
        campaignId: CAMPAIGN,
        expectedEpoch: 1,
        generation: GENERATION,
    ...
    const rolledBack = await rollbackEncounterLocalAuthority(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      expectedEpoch: 1,
      generation: GENERATION,
  ...
  it('rejects missing, journaled, mismatched, and atomically aborted local generations', async () => {
    ...
    await expect(
      commitEncounterLocalCutover(database, {
        ...
        initialDocuments: [
          document({ legacyId: 'encounter-a' }),
          document({ legacyId: 'encounter-b', campaignId: 'wrong' }),
        ],
      })
    ).rejects.toThrow(
  ...
  it('marks cloud authority with epoch CAS and makes local cutover replay idempotent', async () => {
    const database = await openRollkeeperDatabase();
    await seedReady(database);
    const local = await commitEncounterLocalCutover(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      generation: GENERATION,
      confirmed: true,
    ...
    await expect(
      markEncounterCloudAuthority(database, {
        namespace: NAMESPACE,
        campaignId: CAMPAIGN,
    ...
    const cloud = await markEncounterCloudAuthority(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      expectedLocalEpoch: 1,
      cloudEpoch: 1,
  ...
  it('supersedes matching accepted versions and rebases divergent ones per encounter', async () => {
    const database = await openRollkeeperDatabase();
    await seedReady(database);
    await commitEncounterLocalCutover(database, {
      namespace: NAMESPACE,
    ...
    // encounter-a: outbox entry matches the accepted server fingerprint -> superseded
    outbox.put({
      ...document({
        legacyId: 'encounter-a',
        contentFingerprint: 'a'.repeat(64),
    ...
    // encounter-b: local edit diverged from the accepted version -> rebased + un-paused
    ...
    // encounter-c: not part of the accepted batch -> base version untouched
    outbox.put({
      ...document({
        legacyId: 'encounter-c',
        baseServerVersion: 4,
    ...
    outbox.put({
      ...document({ legacyId: 'encounter-z', campaignId: 'campaign-z' }),
    ...
    expect(
      await requestResult(
        documents.get([NAMESPACE, 'encounter_definition', 'encounter-a'])
      )
    ).toMatchObject({ baseServerVersion: 1, cutoverEpoch: 3 });
    expect(
      await requestResult(
        documents.get([NAMESPACE, 'encounter_definition', 'encounter-b'])
      )
    ).toMatchObject({
      baseServerVersion: 2,
      cutoverEpoch: 3,
    ...
    expect(
      await requestResult(
        documents.get([NAMESPACE, 'encounter_definition', 'encounter-c'])
      )
    ).toMatchObject({ baseServerVersion: 0, cutoverEpoch: 1 });
    expect(
      await requestResult(
        documents.get([NAMESPACE, 'encounter_definition', 'encounter-missing'])
      )
    ).toBeUndefined();
  ...
  it('keeps only the newest unresolved entry per encounter at cloud activation', async () => {
    const database = await openRollkeeperDatabase();
    await seedReady(database);
    await commitEncounterLocalCutover(database, {
      namespace: NAMESPACE,
    ...
    // Three paused edits of encounter-a: only the highest local revision survives.
    outbox.put({
      ...document({
        legacyId: 'encounter-a',
        localRevision: 2,
    ...
    // encounter-b: the newest of two paused edits already matches the cloud
  ...
  it('hydrates a new device from the cloud document set and preserves a divergent local candidate', async () => {
    ...
    await expect(
      enrollEncounterCloudDevice(database, {
        namespace: NAMESPACE,
        campaignId: CAMPAIGN,
        campaignCode: 'ABC123',
        deviceId: 'device-a',
    ...
    const authority = await enrollEncounterCloudDevice(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      campaignCode: 'ABC123',
      deviceId: 'device-a',
    ...
    expect(
      await requestResult(
        transaction
          ...
          .get([NAMESPACE, 'encounter_definition', 'encounter-a'])
      )
    ).toMatchObject({
      operation: 'replace',
      localRevision: 1,
    ...
    expect(
      await requestResult(
        transaction
          .objectStore('documents')
          .get([NAMESPACE, 'encounter_definition', 'encounter-b'])
      )
    ).toMatchObject({
      operation: 'delete',
      payload: null,
    ...
    expect(conflicts).toMatchObject([
      {
        conflictId: `encounter-enrollment:${NAMESPACE}:${CAMPAIGN}:device-a`,
        legacyId: 'ABC123',
        family: 'encounter_definition',
    ...
    expect(
      await requestResult(
        transaction
          .objectStore('meta')
          .get(
            `device-enrollment:${NAMESPACE}:encounter_definition:${CAMPAIGN}:device-a`
          )
      )
    ).toMatchObject({
      previewFingerprint: 'a'.repeat(64),
  ...
  it('rejects unsafe enrollment states', async () => {
    ...
    await expect(
      enrollEncounterCloudDevice(database, { ...base, epoch: 0 })
    ...
    await expect(
      enrollEncounterCloudDevice(database, base)
    ).resolves.toMatchObject({
      authority: 'postgres',
    });
    await expect(enrollEncounterCloudDevice(database, base)).rejects.toThrow(
      /already/i
    );
    database.close();
  });
  ...
  it('enrolls after a rollback left a localStorage pointer behind', async () => {
    ...
    await rollbackEncounterLocalAuthority(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      expectedEpoch: 1,
      generation: GENERATION,
