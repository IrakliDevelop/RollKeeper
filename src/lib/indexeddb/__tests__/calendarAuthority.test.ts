...
async function seedReady(database: IDBDatabase) {
  ...
  transaction.objectStore('meta').put({
    key: `migration-state:${NAMESPACE}:calendar:${CAMPAIGN}`,
    state: 'CUTOVER_READY',
    runId: GENERATION,
  });
  transaction.objectStore('kvGenerations').put({
    ...
    presence: true,
    rawValue: '{"state":{"calendars":[]},"version":3}',
  });
  await transactionComplete(transaction);
}

describe('calendar local authority', () => {
  ...
  it('requires every safety gate and explicit confirmation', async () => {
    ...
    for (const field of Object.keys(gates) as Array<keyof typeof gates>) {
      await expect(
        commitCalendarLocalCutover(database, {
          namespace: NAMESPACE,
          campaignId: CAMPAIGN,
          generation: GENERATION,
          confirmed: true,
  ...
  it('atomically advances only the scoped family epoch', async () => {
    const database = await openRollkeeperDatabase();
    await seedReady(database);
    const authority = await commitCalendarLocalCutover(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      generation: GENERATION,
      confirmed: true,
    ...
    expect(await readCalendarAuthority(database, NAMESPACE, CAMPAIGN)).toEqual(
      authority
    );
    expect(await readCalendarAuthority(database, NAMESPACE, 'other')).toEqual({
  ...
  it('rolls back through a new epoch only from a verified current generation', async () => {
    ...
    await expect(
      rollbackCalendarLocalAuthority(database, {
        namespace: NAMESPACE,
        campaignId: CAMPAIGN,
        expectedEpoch: 1,
        generation: GENERATION,
    ...
    const rolledBack = await rollbackCalendarLocalAuthority(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      expectedEpoch: 1,
      generation: GENERATION,
  ...
  it('hydrates a new device only after explicit preview and preserves a divergent local candidate', async () => {
    const database = await openRollkeeperDatabase();
    await expect(
      enrollCalendarCloudDevice(database, {
        namespace: NAMESPACE,
        campaignId: CAMPAIGN,
        campaignCode: 'ABC123',
        deviceId: 'device-a',
    ...
    const authority = await enrollCalendarCloudDevice(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      campaignCode: 'ABC123',
      deviceId: 'device-a',
  ...
  it('marks cloud authority with epoch CAS and makes local cutover replay idempotent', async () => {
    const database = await openRollkeeperDatabase();
    await seedReady(database);
    const local = await commitCalendarLocalCutover(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      generation: GENERATION,
      confirmed: true,
    ...
    await expect(
      markCalendarCloudAuthority(database, {
        namespace: NAMESPACE,
        campaignId: CAMPAIGN,
    ...
    const cloud = await markCalendarCloudAuthority(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
  ...
  it('preserves a newer local working copy and outbox when cloud activation accepts the staged version', async () => {
    ...
    await markCalendarCloudAuthority(database, {
      namespace: NAMESPACE,
      campaignId: CAMPAIGN,
      expectedLocalEpoch: 1,
      cloudEpoch: 1,
  ...
  it('supersedes stale intermediates when the accepted version matches the current working copy', async () => {
    ...
    for (const [mutationId, fingerprint, localRevision, currentTime] of [
      ['intermediate', 'b'.repeat(64), 2, 1],
      ['accepted', 'a'.repeat(64), 3, 2],
    ] as const) {
      outbox.put({
        namespace: NAMESPACE,
        campaignId: CAMPAIGN,
  ...
  it('rejects unsafe enrollment states and supports an exact tombstone hydration', async () => {
    ...
    await expect(
      enrollCalendarCloudDevice(database, { ...base, epoch: 0 })
    ...
    await expect(
      enrollCalendarCloudDevice(database, base)
    ).resolves.toMatchObject({ authority: 'postgres' });
    await expect(enrollCalendarCloudDevice(database, base)).rejects.toThrow(
      /already/i
    );
    database.close();
  });
