...
export type NpcOperation = 'create' | 'replace' | 'delete';
export type NpcWorkState =
  | 'queued'
  | 'inflight'
  | 'retry'
  | 'offline'
...
export interface NpcDocument extends NpcMutation {
  family: 'npc';
  deletedAt: string | null;
}

export interface NpcOutboxEntry extends NpcMutation {
  family: 'npc';
  mutationId: string;
  state: NpcWorkState;
  attemptCount: number;
  ...
  pausedFromState?: Exclude<NpcWorkState, 'paused'>;
}

export interface NpcTombstone {
  namespace: StorageNamespace;
  family: 'npc';
  campaignId: string;
  legacyId: string;
...
const TERMINAL_STATES = new Set<NpcWorkState>(['acknowledged', 'superseded']);

function isTerminal(state: NpcWorkState): boolean {
  return TERMINAL_STATES.has(state);
}

function matches(
  entry: Pick<NpcOutboxEntry, 'namespace' | 'campaignId' | 'family'>,
  namespace: StorageNamespace,
  campaignId: string
) {
  return (
...
export class IndexedDbNpcRepository {
  ...
  > {
    ...
    try {
      const key = [mutation.namespace, 'npc', mutation.legacyId];
      const documents = transaction.objectStore('documents');
      const outbox = transaction.objectStore('outbox');
      const tombstones = transaction.objectStore('tombstones');
      const [current, tombstone, pending] = await Promise.all([
        requestResult(documents.get(key)) as Promise<NpcDocument | undefined>,
        requestResult(tombstones.get(key)) as Promise<NpcTombstone | undefined>,
        requestResult(outbox.getAll()) as Promise<NpcOutboxEntry[]>,
      ]);
      ...
      for (const entry of pending) {
        if (
          ...
          const superseded: NpcOutboxEntry = {
            ...entry,
            state: 'superseded',
            lastError: null,
            inflightAt: null,
  ...
  async getDocument(namespace: StorageNamespace, legacyId: string) {
    ...
    const value = (await requestResult(
      transaction.objectStore('documents').get([namespace, 'npc', legacyId])
    )) as NpcDocument | undefined;
    await transactionComplete(transaction);
    return value ?? null;
  }
  ...
  async getTombstone(namespace: StorageNamespace, legacyId: string) {
    ...
    const value = (await requestResult(
      transaction.objectStore('tombstones').get([namespace, 'npc', legacyId])
    )) as NpcTombstone | undefined;
    await transactionComplete(transaction);
    return value ?? null;
  }
  ...
  private async changePause(
    ...
    const values = (await requestResult(store.getAll())) as NpcOutboxEntry[];
  ...
  async acknowledge(
    ...
    const documents = transaction.objectStore('documents');
    const key = [entry.namespace, 'npc', entry.legacyId];
  ...
  async preserveCloudConflict(entry: NpcOutboxEntry, cloudCandidate: unknown) {
    const transaction = this.database.transaction('conflicts', 'readwrite');
    transaction.objectStore('conflicts').put({
      conflictId: `npc:${entry.namespace}:${entry.campaignId}:${entry.legacyId}:${entry.mutationId}`,
  ...
  async applyAcceptedCloudVersion(input: {
    ...
    if (
      ...
      transaction.abort();
      await completed.catch(() => undefined);
      throw new Error('Unresolved local NPC work blocks cloud hydration');
    }
    const store = transaction.objectStore('documents');
    const tombstones = transaction.objectStore('tombstones');
    const key = [input.namespace, 'npc', input.legacyId];
