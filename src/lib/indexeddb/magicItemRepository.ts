...
export type MagicItemOperation = 'create' | 'replace' | 'delete';
export type MagicItemWorkState =
  | 'queued'
  | 'inflight'
  | 'retry'
  | 'offline'
...
export interface MagicItemMutation {
...
export interface MagicItemDocument extends MagicItemMutation {
  family: 'magic_item';
  deletedAt: string | null;
}

export interface MagicItemOutboxEntry extends MagicItemMutation {
  family: 'magic_item';
  mutationId: string;
  state: MagicItemWorkState;
  attemptCount: number;
  nextAttemptAt: number;
  inflightAt: string | null;
  lastError: string | null;
  pausedFromState?: Exclude<MagicItemWorkState, 'paused'>;
}

export interface MagicItemTombstone {
  namespace: StorageNamespace;
  family: 'magic_item';
  campaignId: string;
  legacyId: string;
  ...
  deletedAt: string;
  mutationId: string;
  beforeImage: MagicItemDocument | null;
}
...
export class IndexedDbMagicItemRepository {
  ...
  > {
    ...
    try {
      const key = [mutation.namespace, 'magic_item', mutation.legacyId];
      ...
      const [current, tombstone, pending] = await Promise.all([
        requestResult(documents.get(key)) as Promise<
          MagicItemDocument | undefined
        >,
        requestResult(tombstones.get(key)) as Promise<
          MagicItemTombstone | undefined
        >,
        requestResult(outbox.getAll()) as Promise<MagicItemOutboxEntry[]>,
      ]);
  ...
  async getDocument(namespace: StorageNamespace, legacyId: string) {
    ...
    const value = (await requestResult(
      transaction
        ...
        .get([namespace, 'magic_item', legacyId])
    )) as MagicItemDocument | undefined;
    await transactionComplete(transaction);
    return value ?? null;
  }
  ...
  async getTombstone(namespace: StorageNamespace, legacyId: string) {
    ...
    const value = (await requestResult(
      transaction
        ...
        .get([namespace, 'magic_item', legacyId])
    )) as MagicItemTombstone | undefined;
    await transactionComplete(transaction);
    return value ?? null;
  }
  ...
  async acknowledge(
    ...
    const documents = transaction.objectStore('documents');
    const key = [entry.namespace, 'magic_item', entry.legacyId];
  ...
  async preserveCloudConflict(
    ...
    transaction.objectStore('conflicts').put({
      conflictId: `magic_item:${entry.namespace}:${entry.campaignId}:${entry.legacyId}:${entry.mutationId}`,
  ...
  async applyAcceptedCloudVersion(input: {
    ...
    if (
      ...
      await completed.catch(() => undefined);
      throw new Error(
        'Unresolved local magic item work blocks cloud hydration'
      );
    }
    const store = transaction.objectStore('documents');
    const tombstones = transaction.objectStore('tombstones');
    const key = [input.namespace, 'magic_item', input.legacyId];
