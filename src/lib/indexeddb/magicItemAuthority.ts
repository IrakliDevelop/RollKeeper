...
import type {
  MagicItemDocument,
  MagicItemOutboxEntry,
  MagicItemTombstone,
} from './magicItemRepository';

export interface MagicItemCutoverGates {
...
export type MagicItemAuthority =
  | {
...
export interface MagicItemAcceptedVersion {
  legacyId: string;
  serverVersion: number;
  payloadFingerprint: string;
}

export interface MagicItemEnrollmentDocument {
  legacyId: string;
...
function keys(namespace: StorageNamespace, campaignId: string) {
  const scope = `${namespace}:magic_item:${campaignId}`;
  return {
...
export async function readMagicItemAuthority(
  database: IDBDatabase,
  namespace: StorageNamespace,
  campaignId: string
): Promise<MagicItemAuthority> {
  ...
  )) as (MagicItemAuthority & { key: string }) | undefined;
  ...
  return withoutStorageKey(pointer) as MagicItemAuthority;
}

export async function commitMagicItemLocalCutover(
  database: IDBDatabase,
  options: {
    namespace: StorageNamespace;
    campaignId: string;
    generation: string;
    confirmed: boolean;
    gates: MagicItemCutoverGates;
    now: () => string;
    initialDocuments?: MagicItemDocument[];
    testHooks?: { abortPointerTransaction?: boolean };
  }
) {
  if (!options.confirmed)
    throw new Error('Magic item cutover requires confirmation');
  if (Object.values(options.gates).some(value => !value)) {
    throw new Error('Magic item cutover gate is not satisfied');
  }
  ...
  if (state?.state !== 'CUTOVER_READY' || state.runId !== options.generation) {
    ...
    throw new Error('Magic item generation is not CUTOVER_READY');
  }
  ...
  if (
    !rows.some(
      row =>
        ...
        row.key === 'rollkeeper-dm-magic-item-library'
    )
  ) {
    transaction.abort();
    await completed.catch(() => undefined);
    throw new Error('Magic item generation is missing');
  }
  ...
  if (
    journal.some(
      row =>
        row.namespace === options.namespace &&
        row.generation === options.generation &&
        row.family === 'magic_item'
    )
  ) {
    transaction.abort();
    await completed.catch(() => undefined);
    throw new Error('Magic item journal is not empty');
  }
  ...
  for (const initial of options.initialDocuments ?? []) {
    if (
      ...
      initial.family !== 'magic_item'
    ) {
      transaction.abort();
      await completed.catch(() => undefined);
      throw new Error(
        'Initial magic item document scope does not match cutover'
      );
    }
  ...
  try {
    ...
    throw new Error('Atomic magic item pointer transaction aborted');
  }
  return authority;
}

export async function rollbackMagicItemLocalAuthority(
  database: IDBDatabase,
  options: {
    namespace: StorageNamespace;
    campaignId: string;
  ...
  if (!options.confirmed)
    throw new Error('Magic item rollback requires confirmation');
  ...
  if (
    ...
    throw new Error('Stale magic item authority epoch');
  }
  const rolledBack = {
...
export async function markMagicItemCloudAuthority(
  ...
  options: {
    ...
    cloudEpoch: number;
    now: () => string;
    acceptedVersions?: MagicItemAcceptedVersion[];
  }
) {
  const scoped = keys(options.namespace, options.campaignId);
  const transaction = database.transaction(
  ...
  if (
    ...
    await transactionComplete(transaction).catch(() => undefined);
    throw new Error(
      'Local magic item authority is not ready for cloud activation'
    );
  }
  const authority = {
    ...current,
...
export async function enrollMagicItemCloudDevice(
  database: IDBDatabase,
  options: {
    namespace: StorageNamespace;
    campaignId: string;
    ...
    documents: readonly MagicItemEnrollmentDocument[];
  ...
  if (current && current.authority !== 'localStorage') {
    transaction.abort();
    await completed.catch(() => undefined);
    throw new Error('This browser already has magic item authority');
  }
  ...
  for (const document of options.documents) {
    ...
    if (document.tombstoned) {
      tombstones.put({
        ...
        beforeImage: null,
      } satisfies MagicItemTombstone);
    }
  }
  if (options.localCandidate) {
    transaction.objectStore('conflicts').put({
      conflictId: `magic-item-enrollment:${options.namespace}:${options.campaignId}:${options.deviceId}`,
      namespace: options.namespace,
      campaignId: options.campaignId,
      family: 'magic_item',
  ...
  meta.put({ key: scoped.epoch, value: authority.epoch });
  meta.put({
    key: `device-enrollment:${options.namespace}:magic_item:${options.campaignId}:${options.deviceId}`,
