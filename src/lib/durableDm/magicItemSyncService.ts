...
export interface MagicItemCloudAcknowledgement {
...
export interface MagicItemCloudGateway {
  put(entry: MagicItemOutboxEntry): Promise<MagicItemCloudAcknowledgement>;
}

export class MagicItemSyncService {
  constructor(
    private readonly options: {
      enabled: boolean;
      repository: IndexedDbMagicItemRepository;
      gateway: MagicItemCloudGateway;
    }
  ) {}

  async commit(mutation: MagicItemMutation) {
  ...
  async flush(namespace: MagicItemMutation['namespace'], campaignId: string) {
    if (!this.options.enabled) return { status: 'disabled' as const };
  ...
  private async push(entry: MagicItemOutboxEntry) {
