...
export const combatLogArchiveAdapter: DurableFamilyAdapter<CombatLogArchiveManifest> =
  {
    ...
    async prepareIndexedDb(context) {
      ...
      assertFamilySelectionMatchesRun('combat_log_archive', context);
    ...
    async repairAuthority(context) {
      ...
      let rawPointer;
      try {
        rawPointer = await readCombatLogArchiveAuthority(
          database,
          namespace,
          context.campaignId
        );
      ...
      const decision = await decideAuthorityRepair({
        ...
        evidence: {
          ...
          async verifyPostgresParity() {
            ...
            // also bumps the epoch — `rollbackCombatLogArchiveLocalAuthority` writes
