...
} from '@/components/ui/campaign/EncounterSyncControls';
import { browserRecoveryRepository } from '@/lib/browserRecoveryRepository';
import { encounterAdapter } from '@/lib/durableDm/adapters/encounterAdapter';
import type {
...
import * as encounterLegacyAuthorityModule from '@/lib/durableDm/encounterLegacyAuthority';
import { encounterAuthorityKey } from '@/lib/durableDm/encounterLegacyAuthority';
...
import * as encounterAuthorityModule from '@/lib/indexeddb/encounterAuthority';
import * as encounterMigrationModule from '@/lib/indexeddb/encounterMigration';
import {
...
import { DEFAULT_COMBAT_CONFIG, type Encounter } from '@/types/encounter';
...
function encounterFixture(index: number): Encounter {
  return {
    id: `enc-${index}`,
    name: `Seed Encounter ${index}`,
...
// see `harnesses/npc.tsx`'s and `harnesses/magicItem.tsx`'s identical
// comment. `name` is `'Encounters'` so the card's `/Use Encounters/` button
// regex (`Use {item.name} ({item.displayCode})`) matches.
 ...
 * `ConformanceHarness` Task 7 declares. `seedWithActiveEncounter` and
 * `rawEnvelopeFields` are the brief's two mandated family-local extensions.
 */
export interface EncounterConformanceHarness extends CardParityHarness {
  /** Every action string sent to `/api/encounter-sync`, live. */
  ...
  /** Seeds `count` encounters instead of the default two. */
  seedWithItems(count: number): Promise<MigrationRunContext>;
  /**
   * Brief-mandated: seeds a SINGLE encounter with `isActive: true`, so
   * `previewManifest` reports the `active-encounter` blocker and
   * `prepareIndexedDb` refuses. Family-local because no other family in this
   * slice has a blocker whose live-combat semantics require a dedicated seed.
   */
  seedWithActiveEncounter(): Promise<MigrationRunContext>;
  /**
   * Brief-mandated: reads the named top-level fields straight off the
   * PERSISTED `rollkeeper-encounter-data` envelope's `state` object (never
   * through `buildEncounterManifest`, which never even looks at them) and
   * returns a JSON string of `{field: value}` pairs in the given order — a
   * STRING, which is a primitive, so the brief's `toBe` (`Object.is`)
   * comparison across two calls is valid Object.is-equality on two separately
   * constructed strings with identical content. `combatConfig` is an object,
   * so no single field here can itself be the returned primitive; ruling
   * R3.1 anticipated this ("if it cannot, the assertion changes to
   * `toEqual`") but returning one JSON string of the whole selection keeps
   * the brief's literal `toBe` correct instead.
   */
  rawEnvelopeFields(fields: string[]): string;
   ...
   * (`EncounterSyncControls.hooks.ts`'s own `assertWorkingCopyUnchanged`
   * checks only `contentFingerprint`, `:1052-1057`) — declared adapter-only,
   * not removed.
   */
   ...
   * `EncounterSyncControls.hooks.ts`'s own `prepare()` (`:894-897`).
   */
...
export function createEncounterHarness(): EncounterConformanceHarness {
  vi.restoreAllMocks();
  ...
  // three neighbouring null-checks (mirrors `harnesses/npc.tsx` and
  // `harnesses/magicItem.tsx`).
            ...
            for (const item of items) {
              ...
              serverDocuments.set(item.legacyId, {
                ...
                payload: item.tombstoned
                  ? null
                  : (item.payload as EncounterPayload),
              });
            }
  ...
  const realMarkEncounterCloudAuthority =
    encounterAuthorityModule.markEncounterCloudAuthority;
  vi.spyOn(
    encounterAuthorityModule,
    'markEncounterCloudAuthority'
  ).mockImplementation(async (...args) => {
    trace.push('mark-cloud-authority');
    recordLibraryCall('markEncounterCloudAuthority', args);
    return realMarkEncounterCloudAuthority(...args);
  });

  const realCommitEncounterLocalCutover =
    encounterAuthorityModule.commitEncounterLocalCutover;
  vi.spyOn(
    encounterAuthorityModule,
    'commitEncounterLocalCutover'
  ).mockImplementation(async (...args) => {
    cutoverSink?.push('cutover');
    recordLibraryCall('commitEncounterLocalCutover', args);
    return realCommitEncounterLocalCutover(...args);
  });

  const realRollbackEncounterLocalAuthority =
    encounterAuthorityModule.rollbackEncounterLocalAuthority;
  vi.spyOn(
    encounterAuthorityModule,
    'rollbackEncounterLocalAuthority'
  ).mockImplementation(async (...args) => {
    recordLibraryCall('rollbackEncounterLocalAuthority', args);
    return realRollbackEncounterLocalAuthority(...args);
  });

  const realRunEncounterIndexedDbMigration =
    encounterMigrationModule.runEncounterIndexedDbMigration;
  vi.spyOn(
    encounterMigrationModule,
    'runEncounterIndexedDbMigration'
  ).mockImplementation(async (...args) => {
    recordLibraryCall('runEncounterIndexedDbMigration', args);
    return realRunEncounterIndexedDbMigration(...args);
  });

  const realWriteEncounterAuthorityMarker =
    encounterLegacyAuthorityModule.writeEncounterAuthorityMarker;
  vi.spyOn(
    encounterLegacyAuthorityModule,
    ...
    return realWriteEncounterAuthorityMarker(...args);
  });
  ...
  async function seedWithRawItems(
    ...
    // envelope AFTER, so the explicit raw write wins. `combatConfig` and
    // `activeEncounterId` are written on BOTH sides so the "byte-identical
    // across a cutover" test has something concrete to compare — this is
    // the "defensive raw-envelope write" R7 requires be kept even though
    ...
    useEncounterStore.setState({
      encounters: rawItems as unknown as Encounter[],
      encounterTombstones: {},
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        version: ENCOUNTER_PERSIST_VERSION,
        state: {
          encounters: rawItems,
          encounterTombstones: {},
          activeEncounterId: null,
    ...
    // `EncounterSyncProvider` (like `NpcSyncProvider`) reads its `campaign`
    // object live from `useDmStore` by `campaignCode`, so every seed carries
    // a matching campaign entry too.
    useDmStore.setState({
      campaigns: [
        {
          code: CAMPAIGN_CODE,
  ...
  async function runChainThroughLocalCutover(context: MigrationRunContext) {
    await encounterAdapter.selectFamily(context);
    const prepared = await encounterAdapter.prepareIndexedDb(context);
    await encounterAdapter.commitLocalCutover(context, {
  ...
  async function commitExtraDocument(legacyId: string) {
    ...
    try {
      ...
      await repository.commit({
        ...
        cutoverEpoch: 1,
        operation: 'create',
        payload: encounterPayloadFromEncounter({
          ...encounterFixture(999),
          id: legacyId,
  ...
  return {
    ...
    seed: () =>
      seedWithRawItems([
        encounterFixture(1) as unknown as Record<string, unknown>,
        encounterFixture(2) as unknown as Record<string, unknown>,
      ]),
    seedWithBlocker: () =>
      seedWithRawItems([
        {
    ...
    seedWithItems: (count: number) =>
      seedWithRawItems(
        Array.from({ length: count }, (_, index) =>
          encounterFixture(index + 1)
        ) as unknown as Record<string, unknown>[]
      ),
    seedWithActiveEncounter: () =>
      seedWithRawItems([
        {
          ...(encounterFixture(1) as unknown as Record<string, unknown>),
          isActive: true,
    ...
    rawEnvelopeFields(fields: string[]): string {
      ...
      if (!raw) throw new Error('No legacy envelope to read');
      const parsed = JSON.parse(raw) as { state?: Record<string, unknown> };
      const selection: Record<string, unknown> = {};
      for (const field of fields) selection[field] = parsed.state?.[field];
    ...
    async runChainThroughCloudActivation(context) {
      ...
      const manifest = await encounterAdapter.previewManifest(context);
      await encounterAdapter.activateCloud(context, manifest);
    },
    ...
    seedDeviceId(deviceId) {
      // Matches the adapter's `deviceIdFor('encounter', ...)` key prefix,
      // read straight off the shipped card's own persisted key
      // (`EncounterSyncControls.hooks.ts:1067`, `:1199`, `:1633`:
      // `` `rollkeeper:encounter-device:${accountId}:${campaignId}` ``).
      localStorage.setItem(
        `rollkeeper:encounter-device:${ACCOUNT_ID}:${CAMPAIGN_ID}`,
        deviceId
      );
    },
    ...
    async deleteWorkingCopy(legacyId) {
      ...
      try {
        ...
        await repository.commit({
          ...
          contentFingerprint: await fingerprintEncounterTombstone(legacyId),
          updatedAt: NOW,
        });
      } finally {
        database.close();
    ...
    async pointerState() {
      const database = await openRollkeeperDatabase();
      try {
        const pointer = await encounterAuthorityModule.readEncounterAuthority(
          database,
          NAMESPACE,
          CAMPAIGN_ID
        );
    ...
    async mutateLegacyEnvelope() {
      ...
      const parsed = JSON.parse(raw) as {
        state: { encounters: { id: string; name?: string }[] };
      };
      const encounter = parsed.state.encounters.find(
        entry => entry.id === 'enc-1'
      );
      if (!encounter) throw new Error('Seeded encounter is missing');
      encounter.name = `${encounter.name ?? ''} (edited)`;
      localStorage.setItem(ENCOUNTER_STORAGE_KEY, JSON.stringify(parsed));
    },

    /**
     * Reads the PERSISTED `rollkeeper-encounter-data` envelope directly
     * (never the in-memory `useEncounterStore` state, and never IndexedDB)
     * — this is what makes the marker-before-restore write ORDER observable
     * at all. `encounter_definition` is a multi-record family whose
     * `encounters` array is FLAT across every campaign, so this returns the
     * ordered list persisted for `CAMPAIGN_CODE` only (`[]` if absent),
     * matching what `applyEncounterDocuments` leaves for this campaign after
     * filtering every other campaign's rows out untouched.
     */
    async readLegacyStorePayload() {
      const raw = localStorage.getItem(ENCOUNTER_STORAGE_KEY);
      if (!raw) return [];
      const parsed = JSON.parse(raw) as {
        state?: { encounters?: { campaignCode?: string }[] };
      };
      return (parsed.state?.encounters ?? []).filter(
     ...
     * `applyEncounterDocuments` branches, reproducing "edits made during the
     * migrated period" on more than one document and one field:
     *   - `enc-1`'s payload changes (the update branch);
     *   - `enc-2` is tombstoned in the diverged generation, having been live
     *     in the migrated one (the delete/tombstone branch);
     *   - `enc-added-elsewhere` exists in the diverged generation only,
     *     absent from the migrated one (the added branch).
     */
    async divergeCloudGeneration() {
      const enc1 = serverDocuments.get('enc-1');
      if (!enc1) throw new Error('Expected enc-1 in the cloud generation');
      serverDocuments.set('enc-1', {
        ...enc1,
        serverVersion: enc1.serverVersion + 1,
        payloadFingerprint: 'd'.repeat(64),
        payload: {
          ...(enc1.payload as EncounterPayload),
          name: 'Edited on another device',
        },
      });
      const enc2 = serverDocuments.get('enc-2');
      if (enc2) {
        serverDocuments.set('enc-2', {
          ...enc2,
          serverVersion: enc2.serverVersion + 1,
          tombstoned: true,
          payload: null,
          payloadFingerprint: 'e'.repeat(64),
        });
      }
      serverDocuments.set('enc-added-elsewhere', {
        legacyId: 'enc-added-elsewhere',
        serverVersion: 1,
        schemaVersion: 1,
     ...
     * imported from `encounterFamily.ts` (`encounterFromPayload`/
     * `sortEncounters`, the functions the adapter itself uses to build its
     * restore) — restating the projection here, not sharing the adapter's
     * own oracle (ruling R8.4's self-fulfilling-fake pattern).
     */
    expectedLegacyStoreAfterRollback: () => {
      const encounters = documentSnapshot()
        .filter(document => document.payload && !document.tombstoned)
        .map(document => ({
          ...(document.payload as EncounterPayload),
    ...
    recordRollbackOrderInto(sink: string[]) {
      ...
      const realSetState = useEncounterStore.setState;
      vi.spyOn(useEncounterStore, 'setState').mockImplementation(
    ...
    withTombstonedRecord(manifest, legacyId) {
      const clone = structuredClone(
        manifest
      ) as FamilyManifestHandle<EncounterManifest>;
    ...
    async rawDocument(legacyId) {
      const database = await openRollkeeperDatabase();
      try {
        return await new IndexedDbEncounterRepository(database).getDocument(
          NAMESPACE,
          legacyId
        );
      } finally {
    ...
    async replaceWorkingCopyEntirely(missingLegacyId, extraLegacyId) {
      ...
      try {
        ...
        transaction
          .objectStore('documents')
          .delete([NAMESPACE, 'encounter_definition', missingLegacyId]);
        await transactionComplete(transaction);
      } finally {
        database.close();
      }
    ...
    /**
     * Renders the SHIPPED `EncounterSyncControls` card (wrapped in its
     * required `EncounterSyncProvider`) and drives it, by clicking its own
     * buttons, through discovery -> select -> preview -> download recovery
     * file -> verify (via a re-uploaded File built from the downloaded Blob)
     * and select -> prepare -> confirm local cutover -> activate cloud ->
     * verified rollback. Every library call the card makes along the way is
     * captured by the SAME spies `recordedLibraryCalls()` and
     * `allCloudRequestBodies()` expose for the adapter.
     *
     ...
     *     record on the card (`EncounterSyncControls.hooks.ts:1113-1117`) vs
     ...
     *     EITHER side — see `encounterAdapter.ts`'s `previewManifest` doc
     ...
     *   - the card also exposes enrollment/history/restore/remove-account
     *     flows this suite never drives — the adapter has no equivalent
     *     methods, matching every other family in this slice.
     */
    async runCardThroughFullChain() {
      ...
      render(
        <EncounterSyncProvider campaignCode={CAMPAIGN_CODE}>
          <EncounterSyncControls
            campaign={{
              code: CAMPAIGN_CODE,
              name: 'Encounters',
      ...
      fireEvent.click(
        await screen.findByRole('button', {
          name: 'Turn on backup to my account',
        })
      );
      await screen.findByText(
    ...
    currentMarkerRaw() {
      const raw = localStorage.getItem(encounterAuthorityKey(CAMPAIGN_CODE));
