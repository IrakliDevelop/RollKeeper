...
const payload: CombatLogArchivePayload = {
...
describe('CombatLogArchiveSyncService', () => {
  ...
  it('does zero repository or network work while disabled', async () => {
    ...
    const service = new CombatLogArchiveSyncService({
      enabled: false,
  ...
  it('reports local failures, missing outbox rows, and an idle queue without network work', async () => {
    const gateway = { put: vi.fn() };
    const failed = new CombatLogArchiveSyncService({
      enabled: true,
      repository: {
        commit: vi
          .fn()
    ...
    const missing = new CombatLogArchiveSyncService({
      enabled: true,
      repository: {
        commit: vi
          .fn()
  ...
  it('keeps response-loss/offline work durable and marks CAS conflicts without choosing a winner', async () => {
    ...
    const offline = new CombatLogArchiveSyncService({
      enabled: true,
      repository,
    ...
    const [retryEntry] = await repository.listOutbox(
      'user:account-a',
      'campaign-a'
    );
    expect(retryEntry).toEqual(
      expect.objectContaining({
    ...
    expect(retryEntry.nextAttemptAt).toBeGreaterThanOrEqual(beforePush + 2_000);
    ...
    const conflict = new CombatLogArchiveSyncService({
      enabled: true,
      repository,
...
describe('CombatLogArchiveHttpGateway', () => {
  const entry: CombatLogArchiveOutboxEntry = {
    ...mutation,
  ...
  it('posts the outbox entry to the combat log archive sync route', async () => {
    ...
    await expect(new CombatLogArchiveHttpGateway().put(entry)).resolves.toEqual(
      acknowledgement
    );
    expect(fetchSpy).toHaveBeenCalledTimes(1);
    const [url, init] = fetchSpy.mock.calls[0];
    expect(url).toBe('/api/combat-log-sync');
  ...
  it('surfaces conflict and failure statuses on the thrown error', async () => {
    ...
    await expect(
      ...
      message: 'Combat log archive cloud request failed.',
      status: 503,
    });
  });
});
