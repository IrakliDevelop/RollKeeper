...
export interface CombatLogArchiveCloudAcknowledgement {
...
export interface CombatLogArchiveCloudGateway {
  put(
    entry: CombatLogArchiveOutboxEntry
  ): Promise<CombatLogArchiveCloudAcknowledgement>;
}

export class CombatLogArchiveSyncService {
  constructor(
    private readonly options: {
      enabled: boolean;
      repository: IndexedDbCombatLogArchiveRepository;
      gateway: CombatLogArchiveCloudGateway;
    }
  ) {}

  async commit(mutation: CombatLogArchiveMutation) {
  ...
  async flush(
    namespace: CombatLogArchiveMutation['namespace'],
    campaignId: string
  ) {
    if (!this.options.enabled) return { status: 'disabled' as const };
  ...
  private async push(entry: CombatLogArchiveOutboxEntry) {
