...
export interface CampaignSettingsCloudAcknowledgement {
  serverVersion: number;
  cutoverEpoch: number;
  payloadFingerprint: string;
  cloudSaved: true;
  playerView: 'pending' | 'not-applicable';
}

export interface CampaignSettingsCloudGateway {
  put(
    entry: CampaignSettingsOutboxEntry
  ): Promise<CampaignSettingsCloudAcknowledgement>;
}

export class CampaignSettingsSyncService {
  constructor(
    private readonly options: {
      enabled: boolean;
      repository: IndexedDbCampaignSettingsRepository;
      gateway: CampaignSettingsCloudGateway;
    }
  ) {}

  async commit(mutation: CampaignSettingsMutation) {
  ...
  async flush(
    namespace: CampaignSettingsMutation['namespace'],
    campaignId: string
  ) {
    if (!this.options.enabled) return { status: 'disabled' as const };
  ...
  private async push(entry: CampaignSettingsOutboxEntry) {
