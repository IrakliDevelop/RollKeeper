...
export interface CalendarCloudAcknowledgement {
...
export interface CalendarCloudGateway {
  put(entry: CalendarOutboxEntry): Promise<CalendarCloudAcknowledgement>;
}
...
export class CalendarSyncService {
  constructor(
    private readonly options: {
      enabled: boolean;
      repository: IndexedDbCalendarRepository;
      gateway: CalendarCloudGateway;
    }
  ) {}

  async commit(mutation: CalendarMutation) {
  ...
  async flush(namespace: CalendarMutation['namespace'], campaignId: string) {
    if (!this.options.enabled) return { status: 'disabled' as const };
  ...
  private async push(entry: CalendarOutboxEntry) {
