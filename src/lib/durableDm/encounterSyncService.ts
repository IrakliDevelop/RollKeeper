...
export interface EncounterCloudAcknowledgement {
...
export interface EncounterCloudGateway {
  put(entry: EncounterOutboxEntry): Promise<EncounterCloudAcknowledgement>;
}
...
export class EncounterSyncService {
  constructor(
    private readonly options: {
      enabled: boolean;
      repository: IndexedDbEncounterRepository;
      gateway: EncounterCloudGateway;
    }
  ) {}

  async commit(mutation: EncounterMutation) {
  ...
  async flush(namespace: EncounterMutation['namespace'], campaignId: string) {
    if (!this.options.enabled) return { status: 'disabled' as const };
  ...
  private async push(entry: EncounterOutboxEntry) {
