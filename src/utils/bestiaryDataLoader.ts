...
import { RawMonsterData, ProcessedMonster, ProcessedTrait } from '@/types/bestiary';
import { formatSourceForDisplay } from './sourceUtils';
import { parseReferences } from './referenceParser';

// Types for data processing
type StringOrObjectItem = string | { 
  special?: string; 
  note?: string; 
  [key: string]: unknown;
};

type MonsterEntry = string | { 
  type?: string; 
  name?: string; 
...
type ChallengeRating = string | number | { 
  cr?: string; 
  coven?: string; 
  lair?: string;
  [key: string]: unknown;
...
function formatSpeed(speed?: { [type: string]: number | boolean | { number: number, condition: string } }): string {
    if (!speed) return 'N/A';
...
function formatStringOrObjectArray(items: StringOrObjectItem[] | undefined, key: string): string {
        ...
        if (typeof item === 'object' && item !== null) {
            ...
            if (itemKey) {
                if (Array.isArray(itemKey)) {
                    const subItems = itemKey.map(subItem => {
...
function formatSenses(senses: SenseEntry[] | undefined, passivePerception: number): string {
    if (!senses || senses.length === 0) return `passive Perception ${passivePerception}`;
    const sensesStr = senses.map(s => {
...
function processTraits(traits?: { name: string, entries: MonsterEntry[] }[]): ProcessedTrait[] | undefined {
...
function processMonster(monster: RawMonsterData): ProcessedMonster {
    ...
    const typeData = typeof monster.type === 'object' ? monster.type : { type: monster.type || 'unknown' };

    return {
        id,
        name: monster.name,
        size: Array.isArray(monster.size) ? monster.size : [monster.size ?? 'Unknown'],
        type: typeData.type,
        alignment: monster.alignment?.map(a => (typeof a === 'object' ? a.alignment || a.entry : a)).join(', ') || 'Unaligned',
        ...
        wis: monster.wis || 10,
        cha: monster.cha || 10,
        saves: monster.save ? Object.entries(monster.save).map(([key, val]) => `${key.toUpperCase()} ${val}`).join(', ') : '',
        skills: monster.skill ? Object.entries(monster.skill).map(([key, val]) => `${key.charAt(0).toUpperCase() + key.slice(1)} ${val}`).join(', ') : '',
        resistances: formatStringOrObjectArray(monster.resist, 'resist'),
        immunities: formatStringOrObjectArray(monster.immune, 'immune'),
        vulnerabilities: formatStringOrObjectArray(monster.vulnerable, 'vulnerable'),
        senses: formatSenses(monster.senses as SenseEntry[] | undefined, monster.passive ?? 0),
      ...
      if (data.monster && Array.isArray(data.monster)) {
        for (const rawMonster of data.monster) {
          try {
            ...
            console.error(`Error processing monster "${rawMonster.name}" in ${file}:`, error);
          }
        }
      }
    } catch (error) {
