...
interface DMStoreState {
  ...
  getPlayerCharacterById: (campaignId: string, characterId: string) => PlayerCharacterReference | null;
  ...
  importPlayerCharacter: (campaignId: string, characterData: CharacterState, importSource: CharacterImportSource) => string;
  updatePlayerCharacter: (campaignId: string, characterId: string, updates: Partial<PlayerCharacterReference>) => void;
  removePlayerCharacter: (campaignId: string, characterId: string) => void;
  syncPlayerCharacter: (campaignId: string, characterId: string, newCharacterData: CharacterState) => void;
  
  // Encounter management
  createEncounter: (campaignId: string, encounter: Omit<CombatEncounter, keyof DMEntityBase | 'campaignId'>) => string;
  updateEncounter: (encounterId: string, updates: Partial<CombatEncounter>) => void;
  ...
  // Combat logging
  addCombatLogEntry: (encounterId: string, entry: Omit<CombatLogEntry, 'id' | 'timestamp'>) => void;
  
  // Session management
  createSession: (campaignId: string, session: Omit<Session, keyof DMEntityBase | 'campaignId'>) => string;
  updateSession: (sessionId: string, updates: Partial<Session>) => void;
  deleteSession: (sessionId: string) => void;
  
  // Notes management
  createNote: (campaignId: string, note: Omit<CampaignNote, keyof DMEntityBase | 'campaignId'>) => string;
...
const createDMEntity = (base: Omit<DMEntityBase, keyof DMEntityBase>): DMEntityBase => ({
  ...base,
  id: generateId(),
  createdAt: new Date(),
  updatedAt: new Date(),
  ...
  persist(
    (set, get) => ({
      ...
      getActiveEncounter: () => {
        ...
        return campaign.encounters.find(e => e.id === activeEncounterId) || null;
      },
      ...
      getPlayerCharacterById: (campaignId: string, characterId: string) => {
        ...
        return campaign?.playerCharacters.find(pc => pc.id === characterId) || null;
      },
        ...
        set(state => ({
          campaigns: state.campaigns.map(campaign =>
            campaign.id === campaignId
              ? {
                  ...campaign,
                  playerCharacters: [...campaign.playerCharacters, characterRef],
                  updatedAt: new Date()
                }
              : campaign
          ),
        ...
        set(state => ({
          campaigns: state.campaigns.map(campaign =>
            campaign.id === campaignId
              ? {
                  ...campaign,
                  playerCharacters: campaign.playerCharacters.filter(pc => pc.id !== characterId),
                  updatedAt: new Date()
                }
              : campaign
          ),
      ...
      createEncounter: (campaignId, encounterData) => {
        const encounter: CombatEncounter = {
          ...
          settings: { ...DEFAULT_ENCOUNTER_SETTINGS, ...encounterData.settings },
          combatLog: [],
      ...
      removeParticipant: (encounterId, participantId) => {
        set(state => ({
          ...
          selectedParticipantIds: state.selectedParticipantIds.filter(id => id !== participantId),
          lastSaved: new Date()
        }));
      },
