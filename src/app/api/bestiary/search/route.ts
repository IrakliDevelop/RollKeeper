...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters: BestiaryFilters = {
      ...
      alignments: searchParams.get('alignments')?.split(',').filter(Boolean) || [],
      sources: searchParams.get('sources')?.split(',').filter(Boolean) || [],
      crRange: {},
      hasLegendaryActions: searchParams.get('hasLegendaryActions') === 'true' ? true : 
                          searchParams.get('hasLegendaryActions') === 'false' ? false : undefined,
      hasSpellcasting: searchParams.get('hasSpellcasting') === 'true' ? true :
                      searchParams.get('hasSpellcasting') === 'false' ? false : undefined,
      hasConditionImmunities: searchParams.get('hasConditionImmunities') === 'true' ? true :
                             searchParams.get('hasConditionImmunities') === 'false' ? false : undefined,
      hasDamageResistances: searchParams.get('hasDamageResistances') === 'true' ? true :
                           searchParams.get('hasDamageResistances') === 'false' ? false : undefined,
    };
