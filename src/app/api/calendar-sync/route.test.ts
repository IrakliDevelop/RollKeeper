...
function request(body: unknown, headers: Record<string, string> = {}) {
  return new Request('http://rollkeeper.test/api/calendar-sync', {
    method: 'POST',
    headers: {
...
describe('calendar API gate', () => {
  ...
  it('enforces exact Origin and CSRF before database access', async () => {
    vi.stubEnv('SUPABASE_CALENDAR_SYNC_ENABLED', 'true');
  ...
  it('passes only the registered family to owner RPCs', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('list_calendar_document_versions', {
