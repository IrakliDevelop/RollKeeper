...
interface ArmorAutocompleteProps {
  ...
  onSelect: (item: ProcessedArmor) => void;
  loading?: boolean;
  disabled?: boolean;
  placeholder?: string;
  className?: string;
...
export function ArmorAutocomplete({
  ...
  loading = false,
  disabled = false,
  placeholder = 'Search armor database...',
  className = '',
  ...
  const handleSelect = (item: ProcessedArmor) => {
    setQuery('');
    setIsOpen(false);
    onSelect(item);
  };
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="mb-2 flex items-center gap-2">
        <Shield className="text-accent-blue-text-muted h-5 w-5" />
        <h4 className="text-body text-sm font-semibold">
          Search Armor Database
        </h4>
        {loading && (
      ...
      <p className="text-muted mt-2 text-xs">
        Search and select armor to auto-fill the form, or fill manually.
      </p>
    </div>
  );
}
