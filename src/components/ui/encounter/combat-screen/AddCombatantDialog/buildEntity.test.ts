...
describe('encounter entity avatars', () => {
  ...
  it('sets hpSharedWithParty: false at add time for a player who opted out', () => {
    const entity = buildPlayerEntity({
      id: 'player-1',
      name: 'Aria',
      class: 'Rogue',
    ...
    expect(entity.hpSharedWithParty).toBe(false);
  });

  it('sets hpSharedWithParty: true at add time when shareHpWithParty is true or undefined', () => {
    const base = {
      id: 'player-1',
      name: 'Aria',
      class: 'Rogue',
    ...
    expect(
      buildPlayerEntity({ ...base, shareHpWithParty: true }).hpSharedWithParty
    ).toBe(true);
    expect(buildPlayerEntity(base).hpSharedWithParty).toBe(true);
  });
