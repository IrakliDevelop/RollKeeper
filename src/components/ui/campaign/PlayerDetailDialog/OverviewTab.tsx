...
export function OverviewTab({
  ...
  return (
    <div className="space-y-6">
        ...
        <QuickStat
          ...
          icon={
            <Sparkles
              size={14}
              className={
                inspirationCount > 0 ? 'text-accent-amber-text' : 'text-faint'
              }
            />
          }
        />
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          ...
          <div className="flex gap-1.5">
            {Array.from({ length: Math.max(inspirationCount, 3) }, (_, i) => {
      ...
      {/* ── Ability Scores ── */}
      <div>
        <SectionTitle icon={<Gem size={14} />}>Ability Scores</SectionTitle>
        <div className="grid grid-cols-6 gap-2">
          {ABILITY_KEYS.map(key => {
            const score = char.abilities?.[key] ?? 10;
            const mod = calculateModifier(score);
            const saveProf = char.savingThrows?.[key]?.proficient ?? false;
            return (
              <div
                key={key}
              ...
              >
                ...
                <div className="text-heading text-lg font-bold">{score}</div>
            ...
            <div className="bg-surface-secondary rounded-lg p-3">
              <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                {SKILL_ENTRIES.map(({ key, ability }) => {
                  const skill = char.skills?.[key as keyof typeof char.skills];
                  const isProficient = skill?.proficient ?? false;
                  const isExpertise = skill?.expertise ?? false;
                  const abilityScore =
                    char.abilities?.[ability as keyof typeof char.abilities] ??
                    10;
            ...
            <div className="bg-surface-secondary rounded-lg p-3">
              <div className="mb-2 flex items-center justify-between text-sm">
                <span className="text-body">
                  {encumbrance.totalWeight} / {encumbrance.carryCapacity} lbs
                </span>
