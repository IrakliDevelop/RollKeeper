...
export function useHpAmountEditor(addToast: (toast: Omit<ToastData, 'id'>) => void) {
  const applyDamageToCharacter = useCharacterStore(s => s.applyDamageToCharacter);
  const applyHealingToCharacter = useCharacterStore(s => s.applyHealingToCharacter);
  const addTemporaryHPToCharacter = useCharacterStore(s => s.addTemporaryHPToCharacter);
  const [amount, setAmount] = useState('');

  const applyAmount = (apply: (n: number) => void, toastTitle: (n: number) => string) => {
    ...
    const { current, max, temporary } = useCharacterStore.getState().character.hitPoints;
  ...
  return {
    amount,
    setAmount,
    onDamage: () => applyAmount(applyDamageToCharacter, n => `Took ${n} damage`),
