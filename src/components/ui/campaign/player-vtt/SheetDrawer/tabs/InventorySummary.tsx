...
function weightBarClass(rawPercent: number): string {
  if (rawPercent > 100) return 'bg-accent-red-bg';
  if (rawPercent > 66) return 'bg-accent-amber-bg';
  return 'bg-accent-emerald-bg';
}
...
export function InventorySummary({ locked, summary }: InventorySummaryProps) {
  ...
  const commitCurrency = (key: keyof Currency, next: number | undefined) => {
    if (next === undefined) return;
    const current = useCharacterStore.getState().character.currency?.[key] ?? 0;
    const diff = next - current;
    if (diff > 0) addCurrency(key, diff);
    else if (diff < 0) subtractCurrency(key, -diff);
  };
        ...
        {summary.currency.map(coin => (
          <div key={coin.key} className="flex items-center gap-1">
            {locked ? (
              ...
              <NumberInput
                id={`sheet-inventory-currency-${coin.key}`}
                aria-label={COIN_NAMES[coin.key]}
                value={coin.value}
                onChange={v => commitCurrency(coin.key, v)}
                min={0}
                size="sm"
