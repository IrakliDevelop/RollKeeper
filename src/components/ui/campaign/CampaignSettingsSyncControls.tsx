...
import {
  ...
  type CampaignSettingsAuthority,
...
} from '@/lib/indexeddb/campaignSettingsSelection';
...
interface VersionExport {
  ...
  payloadFingerprint: string;
  tombstoned: boolean;
  payload: import('@/types/database.generated').Json | null;
}
...
export function CampaignSettingsSyncControls({ campaign }: Props) {
  ...
  const [manifest, setManifest] = useState<CampaignSettingsManifest | null>(
    null
  );
  ...
  const [authority, setAuthority] = useState<CampaignSettingsAuthority | null>(
    null
  );
  ...
  useEffect(() => {
    const marker = readCampaignSettingsProjectionAuthority(
      localStorage,
      campaign.code
    );
    ...
    const hydrate = async (accountId: string | null) => {
      ...
      try {
        const [localAuthority, document] = await Promise.all([
          ...
          new IndexedDbCampaignSettingsRepository(database).getDocument(
            marker.namespace!,
            campaign.code
          ),
        ]);
        ...
        const fingerprint = await fingerprintCampaignSettingsPayload(
          (document.payload ??
            {}) as CampaignSettingsManifest['records'][number]['payload']
        );
        if (fingerprint !== document.contentFingerprint) {
          setError(
            'Local campaign settings failed fingerprint verification; use history recovery.'
          );
          return;
        }
    ...
    const run = async () => {
      ...
      try {
        ...
        const payload = campaignSettingsPayloadFromCampaign(
          campaign,
          (document?.payload ?? undefined) as
            | CampaignSettingsManifest['records'][number]['payload']
            | undefined
        );
        const fingerprint = await fingerprintCampaignSettingsPayload(payload);
        if (cancelled) return;
        ...
        if (authority.authority === 'indexedDB') {
          ...
          const result = await new CampaignSettingsSyncService({
            enabled: true,
            repository,
            gateway: new CampaignSettingsHttpGateway(),
          }).commit(mutation);
          setStatus(
  ...
  const downloadAndSelect = async () => {
    ...
    if (
      !window.confirm(
        `Recovery download initiated. Select only campaign_settings for ${campaign.name}? This does not cut over local or cloud authority.`
      )
    ) {
      setStatus('Recovery downloaded; data category selection was cancelled.');
      return;
    }
  ...
  const prepare = async () => {
    ...
    try {
      if (
        ...
        throw new Error(
          'Download recovery and explicitly select the data category first.'
        );
      }
  ...
  const activateLocal = async () => {
    ...
    if (
      !window.confirm(
        `Confirm exact manifest ${manifest.fingerprint.slice(0, 12)} and cut only campaign_settings to IndexedDB?`
      )
    )
      return;
    const database = await openRollkeeperDatabase();
    try {
      const current = await buildCampaignSettingsManifest({
        campaignCode: campaign.code,
        rawEnvelope: currentRawEnvelope(),
      });
  ...
  const activateCloud = async () => {
    ...
    if (
      !window.confirm(
        'Stage, revalidate, and atomically activate only campaign_settings in Postgres?'
      )
    )
      return;
    setBusy(true);
    try {
      ...
      const deviceKey = `rollkeeper:campaign-settings-device:${context.accountId}:${workspace.cloudId}`;
      let deviceId = localStorage.getItem(deviceKey);
      if (!deviceId) {
        deviceId = crypto.randomUUID();
        localStorage.setItem(deviceKey, deviceId);
      }
      const begun = await campaignSettingsApi<{ runId: string }>({
      ...
      await assertWorkingCopyUnchanged();
      const activated = await campaignSettingsApi<{ epoch: number }>({
  ...
  const previewEnrollment = async () => {
    ...
    setError(null);
    try {
      const preview = await campaignSettingsApi<EnrollmentPreview>({
      ...
      setStatus(
        preview.authority === 'postgres'
          ...
          : 'The selected campaign settings data is not cloud-authoritative.'
      );
    } catch (cause) {
      setError(
  ...
  const enrollDevice = async () => {
    if (
      ...
      return;
    const local = await buildCampaignSettingsManifest({
      campaignCode: campaign.code,
      rawEnvelope: currentRawEnvelope(),
    });
    if (
    ...
    const database = await openRollkeeperDatabase();
    try {
      const next = await enrollCampaignSettingsCloudDevice(database, {
        namespace: `user:${context.accountId}`,
        campaignId: workspace.cloudId,
        campaignCode: campaign.code,
        deviceId,
  ...
  const loadHistory = async () => {
    if (!workspace?.cloudId) return;
    const result = await campaignSettingsApi<{ versions: VersionMetadata[] }>({
      action: 'history',
      campaignId: workspace.cloudId,
      legacyId: campaign.code,
    });
  ...
  const exportVersion = async (serverVersion: number) => {
    if (!workspace?.cloudId) return;
    const value = await campaignSettingsApi({
      action: 'export-version',
      campaignId: workspace.cloudId,
      legacyId: campaign.code,
      serverVersion,
    });
    downloadJson(`campaign-settings-v${serverVersion}.json`, value);
  };

  const compareLatestVersions = async () => {
    if (!workspace?.cloudId || versions.length < 2) return;
    const result = await campaignSettingsApi<{ identical: boolean }>({
  ...
  const restoreVersion = async (sourceVersion: number) => {
    ...
    if (
      ...
      return;
    const restored = await campaignSettingsApi<{
      serverVersion: number;
      cutoverEpoch: number;
      payloadFingerprint: string;
    }>({
    ...
    const exact = await campaignSettingsApi<VersionExport>({
      action: 'export-version',
      campaignId: workspace.cloudId,
      legacyId: campaign.code,
    ...
    const database = await openRollkeeperDatabase();
    try {
      await new IndexedDbCampaignSettingsRepository(
        database
      ).applyAcceptedCloudVersion({
        namespace: `user:${context.accountId}`,
        campaignId: workspace.cloudId,
        legacyId: campaign.code,
  ...
  const rollback = async () => {
    ...
    const [current, projection] = await Promise.all([
      campaignSettingsApi<EnrollmentPreview>({
        action: 'preview-enrollment',
        campaignId: workspace.cloudId,
      }),
    ...
    const database = await openRollkeeperDatabase();
    try {
      const local = await rollbackCampaignSettingsLocalAuthority(database, {
        namespace: `user:${context.accountId}`,
        campaignId: workspace.cloudId,
        expectedEpoch: authority.epoch,
    ...
    try {
      ...
      if (authority.authority === 'postgres') {
        const deviceId = localStorage.getItem(
          `rollkeeper:campaign-settings-device:${context.accountId}:${workspace.cloudId}`
        );
        if (!deviceId)
          throw new Error(
  ...
  const loadProjectionStatus = async () => {
    if (!workspace?.cloudId) return;
    const next = await campaignSettingsApi<ProjectionStatus>({
      action: 'projection-status',
      campaignId: workspace.cloudId,
    });
    ...
    <Card padding="lg">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Cloud size={20} /> Campaign settings cloud canary
        </CardTitle>
        <CardDescription>
