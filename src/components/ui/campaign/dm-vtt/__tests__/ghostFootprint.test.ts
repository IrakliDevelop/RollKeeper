...
function gridCtx(gridType: 'square' | 'hex'): ToolContext {
  return {
    constraintService: {
      getConstraintInfo: () => ({ gridType, cellSize: 40 }),
    },
  } as unknown as ToolContext;
}

const squareCtx = gridCtx('square');
const hexCtx = gridCtx('hex');
const noGridCtx = {
...
describe('ghostFootprintPx', () => {
  it('returns cells × cellUnit × zoom on a square grid', () => {
    // cellUnit(squareCtx) resolves to cellSize (40) — 2 cells at zoom 1.5
