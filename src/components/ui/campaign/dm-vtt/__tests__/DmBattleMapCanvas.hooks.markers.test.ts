...
/**
 * Undo of a delete vs. duplicate — the two shapes this seam cannot tell apart
 * by looking at the element alone.
 *
 ...
 */
describe('useDmBattleMapCanvas — an undo of a delete is not a duplicate', () => {
  beforeEach(() => {
    vi.stubEnv('NEXT_PUBLIC_BATTLEMAP_RELAY_URL', '');
    useBattleMapStore.setState({
      battleMaps: { [CODE]: { [MAP_ID]: battleMapFixture() } },
  ...
  it('undoing the deletion of a SHARED pin leaves it SHARED, with its ref untouched', () => {
    const { vp, store, result, select } = setup();
  ...
  it('undoing the deletion of a DM-ONLY pin leaves it DM-only, with its ref untouched', () => {
    const { vp, store, result } = setup();
  ...
  it('POSITIVE CONTROL: the same session, the same ref, a NEW id — still a duplicate, marked DM-only and given its own ref', () => {
    const { vp, store, result, select } = setup();
  ...
  it('an add whose id matches a remembered removal but whose REF does not falls through to the fail-closed path', () => {
    const { vp, store, result, select } = setup();
