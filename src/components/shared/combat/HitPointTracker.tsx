    ...
    <div className={containerClasses}>
      {!hideLabels && (
        <div className="flex items-center justify-between">
          <h3 className={`${compact ? 'text-base' : 'text-lg'} font-semibold text-red-800 flex items-center gap-2`}>
      ...
      <div className={`grid grid-cols-3 ${gridSizeClasses}`}>
        <div className="text-center">
          <div className={`border-2 rounded-lg p-2 ${compact ? 'sm:p-2' : 'sm:p-3'} transition-colors ${getCurrentHPBoxColor()} ${minHeightClasses} flex flex-col justify-center`}>
            <div className={`text-xs font-medium mb-1 ${getCurrentHPTextColor()}`}>CURRENT</div>
            {readonly ? (
              <div className={`${compact ? 'text-lg' : 'text-lg sm:text-xl'} font-bold ${getCurrentHPTextColor()}`}>
                {hitPoints.current}
              </div>
            ) : (
              <input
                ...
                className={`${compact ? 'text-lg' : 'text-lg sm:text-xl'} font-bold bg-transparent border-none outline-none text-center w-full ${getCurrentHPTextColor()} leading-tight`}
                min="0"
                max={hitPoints.max}
                style={{ maxWidth: '100%' }}
              />
        ...
        <div className="text-center">
          <div className={`bg-red-100 border-2 border-red-300 rounded-lg p-2 ${compact ? 'sm:p-2' : 'sm:p-3'} relative ${minHeightClasses} flex flex-col justify-center`}>
            ...
            {readonly ? (
              <div className={`${compact ? 'text-lg' : 'text-lg sm:text-xl'} font-bold text-red-900`}>
                {hitPoints.max}
              </div>
            ) : (
              <input
                ...
                className={`${compact ? 'text-lg' : 'text-lg sm:text-xl'} font-bold text-red-900 bg-transparent border-none outline-none text-center w-full leading-tight`}
        ...
        <div className="text-center">
          <div className={`bg-blue-50 border-2 border-blue-200 rounded-lg p-2 ${compact ? 'sm:p-2' : 'sm:p-3'} ${minHeightClasses} flex flex-col justify-center`}>
            <div className="text-xs font-medium text-blue-700 mb-1">
              <span>TEMP</span>
            </div>
            {readonly ? (
              <div className={`${compact ? 'text-lg' : 'text-lg sm:text-xl'} font-bold text-blue-800`}>
                {hitPoints.temporary}
              </div>
            ) : (
              <input
                ...
                className={`${compact ? 'text-lg' : 'text-lg sm:text-xl'} font-bold text-blue-800 bg-transparent border-none outline-none text-center w-full leading-tight`}
                min="0"
                style={{ maxWidth: '100%' }}
              />
            )}
      ...
      {/* Death Saving Throws */}
      {showDeathSaves && (isCharacterDying || isCharacterStabilized || hitPoints.deathSaves) && (
