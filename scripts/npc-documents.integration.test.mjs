...
function statBlockNpc(updatedAt, name = 'Ash the Cult Prophet') {
  return {
    name,
      ...
      traits: [
        {
          id: 'trait-1',
          name: 'Ashen Aura',
          description: 'Cinders swirl around the prophet.',
        },
      ],
      actions: [
        {
      ...
      reactions: [
        {
          id: 'reaction-1',
          name: 'Cinder Ward',
          description: 'Reduce damage by 5.',
        },
      ],
      bonusActions: [
        {
          id: 'bonus-1',
          name: 'Smolder',
          description: 'Ignite a nearby creature.',
        },
      ],
      lairActions: [
    ...
    group: 'Cult',
    tags: ['boss', 'fire'],
    hitDice: { total: 12, used: 2, dieType: 8 },
    ...
    inventory: [
      {
        ...
        magicItem: {
          ...
          rarity: 'Rare',
          charges: [{ id: 'chg-1', current: 3, max: 3 }],
        },
      },
    ],
    ...
    spellcasting: {
      ...
      slotsUsed: { 1: 1 },
      spells: [{ id: 'spell-1', name: 'Fire Bolt', level: 0 }],
    },
    resources: [
      {
        id: 'res-1',
    ...
    abilityUsage: { 'action-1': 1 },
    collapsedSpellSections: ['cantrips'],
    lastDetailTab: 'stats',
...
test('real npc database enforces multi-record staging, CAS, immutable history, isolation, enrollment, and rollback without projection', async () => {
  ...
  const workspace = await rpc(config, 'create_campaign_workspace', owner, {
    ...
    p_name: 'NPC integration',
    p_creation_kind: 'new_workspace',
    p_source_fingerprint: null,
  });
  assert.equal(workspace.response.status, 200);
  const campaignId = workspace.body.campaignId;
  const primaryId = 'npc-a';
  const secondaryId = 'npc-b';
  const first = statBlockNpc('2026-08-02T00:00:00.000Z');
  const second = minimalNpc();
  const totalBytes = canonicalBytes(first) + canonicalBytes(second);
  const recovery = fingerprint({ recovery: true });
  const manifest = fingerprint({ family: 'npc', campaignId });

  const denied = await rpc(config, 'begin_npc_staging', other, {
    p_mutation_id: randomUUID(),
  ...
  const begin = await rpc(config, 'begin_npc_staging', owner, {
    p_mutation_id: randomUUID(),
    p_campaign_id: campaignId,
  ...
  const staged = await rpc(config, 'stage_npc_items', owner, {
    p_mutation_id: randomUUID(),
    p_run_id: begin.body.runId,
    p_items: [
      {
        legacyId: primaryId,
        schemaVersion: NPC_SCHEMA_VERSION,
        payload: first,
  ...
  const confirmations = await Promise.all([
    rpc(config, 'confirm_npc_cutover', owner, confirmBody),
    rpc(config, 'confirm_npc_cutover', owner, confirmBody),
  ]);
  assert.deepEqual(
  ...
  const preview = await rpc(config, 'preview_npc_device_enrollment', owner, {
    p_campaign_id: campaignId,
  });
  ...
  const enrollment = await rpc(config, 'enroll_npc_device', owner, {
    p_mutation_id: randomUUID(),
    p_campaign_id: campaignId,
    p_device_id: randomUUID(),
    p_expected_epoch: 1,
  ...
  const left = statBlockNpc('2026-08-03T00:00:00.000Z', 'Ash the Left Prophet');
  const right = statBlockNpc(
    '2026-08-04T00:00:00.000Z',
    'Ash the Right Prophet'
  );
  const makePut = (payload, base) => ({
    ...
    p_schema_version: NPC_SCHEMA_VERSION,
    p_payload: payload,
    p_payload_fingerprint: fingerprint(payload),
  });
  const race = await Promise.all([
    rpc(config, 'put_npc_document', owner, makePut(left, 1)),
    rpc(config, 'put_npc_document', owner, makePut(right, 1)),
  ]);
  ...
  const replayBody = makePut(
    statBlockNpc('2026-08-05T00:00:00.000Z', 'Ash the Replayed Prophet'),
    2
  );
  const responseLoss = await Promise.all([
    rpc(config, 'put_npc_document', owner, replayBody),
    rpc(config, 'put_npc_document', owner, replayBody),
  ]);
  assert.deepEqual(
  ...
  const history = await rpc(config, 'list_npc_document_versions', owner, {
    p_campaign_id: campaignId,
    p_legacy_id: primaryId,
  });
  ...
  const exact = await rpc(config, 'export_npc_document_version', owner, {
    p_campaign_id: campaignId,
    p_legacy_id: primaryId,
    p_server_version: 1,
  });
  assert.equal(exact.body.payload.monsterStatBlock.actions[0].id, 'action-1');
  assert.equal(exact.body.payload.spellcasting.spells[0].id, 'spell-1');
  assert.equal(exact.body.payload.inventory[0].id, 'npc-item-1');

  const otherHistory = await rpc(config, 'list_npc_document_versions', other, {
    p_campaign_id: campaignId,
    p_legacy_id: primaryId,
  });
  assert.equal(otherHistory.response.status, 403);

  const verified = await rpc(config, 'preview_npc_device_enrollment', owner, {
    p_campaign_id: campaignId,
  });
  assert.equal(verified.body.recordCount, 2);
  const rollback = await rpc(config, 'rollback_npc_family', owner, {
    p_mutation_id: randomUUID(),
    p_campaign_id: campaignId,
    p_expected_epoch: 1,
